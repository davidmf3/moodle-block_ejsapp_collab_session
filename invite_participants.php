...
//DMF-I
$groupid = optional_param('groupid', 0, PARAM_INT);                 // optional groupid, 0 means all groups
$noparticipants = optional_param('noparticipants',1, PARAM_INT);
//DMF-F
...
if ($noparticipants==0){
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('noParticipantsErr', 'block_ejsapp_collab_session'));
}else {
    //DMF-F
if (is_the_user_participating_in_any_session()) {
    ...
    echo $OUTPUT->heading(get_string('cantJoinSessionErr1', 'block_ejsapp_collab_session'));
        ...
        //DMF-I
        $enlarge_collab_conf = get_config('block_ejsapp_collab_session', 'Use_Enlarge');
        //DMF-F
...
//DMF-F
        ...
        //DMF-I
        // limit list to users with some group
        if ($groupid) {
            $wheres[] = "u.id IN (SELECT userid FROM {groups_members} WHERE groupid = :groupid)";
...
//DMF-I
        ...
        $group_records = $DB->get_records_sql($sql, $params);
        $multilang = new filter_multilang($context_course, array('filter_multilang_force_old' => 0));
        $group_name = array(0 => get_string('allgroups'));
        foreach ($group_records as $group_record) {
            $group_name[$group_record->id] = $multilang->filter($group_record->name);
        }

        // <Select group pulldown menu>
        $select = new single_select($baseurl, 'groupid', $group_name, $groupid, null, 'groupmenu');
        $select->set_label(get_string('selectGroupBut', 'block_ejsapp_collab_session'));
        $groupslistcell = new html_table_cell();
        $groupslistcell->attributes['class'] = 'right';
        $groupslistcell->text = $OUTPUT->render($select);
        $controlstable->data[0]->cells[] = $groupslistcell;


        $bookingid=0;
        $remlabaccess = new stdClass;
        $practice = $DB->get_field('block_remlab_manager_exp2prc', 'practiceintro', array('ejsappid' => $labid));
        $remlabaccess->remlab_conf = check_create_remlab_conf($practice);
        if (!($remlabaccess->remlab_conf->free_access == 1)) {
            ...
            foreach ($book_records as $book_record) {
                $book_name[$book_record->id] = $multilang->filter($book_record->name);
                if ($i == 0) {
                    $bookingid = $book_record->id;
                    $i++;
                    //echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'bookingid', 'value' => $bookingid));
                }
            }
        }
...
//DMF-I
        ...
        echo html_writer::start_tag('form', array('action' => "send_messages.php?courseid=$courseid&contextid=$contextid&labid=$labid&localport=$localport&ip=$ip&enlargeport=$enlargeport&enlarge_collab_conf=$enlarge_collab_conf&bookingid=$bookingid", 'method'=>'post', 'id'=>'participantsform')) .
             html_writer::empty_tag('input', array('type' => 'hidden', 'name'=>'sesskey', 'value'=>sesskey())) . html_writer::empty_tag('input', array('type'=>'hidden', 'name'=>'returnto', 'value'=>'s(me())'));
//DMF-F
...
//DMF-I
                    if (!isset($hiddenfields['groups'])) {
                        ...
                        foreach ($usergroups as $group) {
                            $usergroupsname = $usergroupsname . $group->name . ', ';

                        }
                        $data[] = trim($usergroupsname, ', ');
                    }
//DMF-F
        ...
        //DMF-I
        $PAGE->requires->js_call_amd('block_ejsapp_collab_session/buttons_states', 'selectParticipants');
        //DMF-F
